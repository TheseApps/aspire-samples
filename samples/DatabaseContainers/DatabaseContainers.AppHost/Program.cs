...
//UNDOPOSTGRES: var todosDbName = "Todos";

//UNDOPOSTGRES: var postgres = builder.AddPostgres("postgres")
// Set the name of the default database to auto-create on container startup.
//UNDOPOSTGRES: .WithEnvironment("POSTGRES_DB", todosDbName)
// Mount the SQL scripts directory into the container so that the init scripts run.
//UNDOPOSTGRES: .WithBindMount("../DatabaseContainers.ApiService/data/postgres", "/docker-entrypoint-initdb.d")
// Configure the container to store data in a volume so that it persists across instances.
//UNDOPOSTGRES: .WithDataVolume()
//UNDOPOSTGRES: .WithPgWeb()
// Keep the container running between app host sessions.
//UNDOPOSTGRES: .WithLifetime(ContainerLifetime.Persistent);

// Add the default database to the application model so that it can be referenced by other resources.
//UNDOPOSTGRES: var todosDb = postgres.AddDatabase(todosDbName);
...
builder.AddProject<Projects.DatabaseContainers_ApiService>("apiservice")
    //UNDOPOSTGRES: .WithReference(todosDb)
    //UNDOPOSTGRES: .WaitFor(todosDb)
